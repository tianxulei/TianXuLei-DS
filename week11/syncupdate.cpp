...
int main(int argc,char *argv[])
{
  ...
  // 如果starg.remotecols或starg.localcols为空，就用starg.localtname表的全部列来填充。a
...
bool _syncupdate()
{
  ...
  if (starg.synctype==1)
  {
    ...
    if (stmtins.execute()!=0)
    {
      ...
      connloc.rollback();   // 如果这里失败了。可以不用回滚，connection析构会回滚
      return false;
    }
