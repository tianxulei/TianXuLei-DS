...
struct st_arg
{
  ...
  char matchname[101];	//待下载文件匹配规则
  char listfilename[301]; //下载前列出服务器文件名的文件
  int ptype;		//文件下载成功后，服务器处理：1.什么也不做，2.删除，3.备份
  char remotepathbak[301]; //文件下载成功后，服务器文件备份目录。
  char okfilename[301];	//已下载成功文件名清单
  bool checkmtime;	//是否需要检查服务器端文件的时间，true-需要，false-不需要，缺省为false
...
//把ftp.nlist()方法获取到的1ist文件加载到容器vlistfile2中。
bool LoadListFile();
...
void _help()
{
  ...
  printf("<remotepath>/tmp/idc/surfdata</remotepath> 远程服务器存放文件的日录。\n");
  printf("<localpath>/idcdata/surfdata</localpath> 本地文件存放目录。\n");
  ...
  printf("<checkmtime>true</checkmtime> 是否需要检查服务器端文件的时间，true-需要，false-不需要，缺省为false.\n");
  printf("<timeout>80</timeout> 下载文件超时时间，单位：秒，视文件大小和网络而定。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  GetXMLBuffer(strxmlbuffer,"matchname",starg.matchname,100);   //待下载文件匹配规则
  ...
  GetXMLBuffer(strxmlbuffer,"listfilename",starg.listfilename,100);   //本低文件存放的目录
  if(strlen(starg.listfilename)==0)
  { logfile.Write("listfilename is null.\n"); return false; }

  GetXMLBuffer(strxmlbuffer,"ptype",&starg.ptype);   //下载成功后的处理
  if(starg.ptype!=1 && starg.ptype!=2 && starg.ptype!=3)
  { logfile.Write("ptype is error.\n"); return false; }

  GetXMLBuffer(strxmlbuffer,"remotepathbak",starg.remotepathbak,300);   //备份路径
  if(starg.ptype==3 && strlen(starg.remotepathbak)==0)
  { logfile.Write("remotepathbak is null.\n"); return false; } 

  GetXMLBuffer(strxmlbuffer,"okfilename",starg.okfilename,300);   //已下载文件清单
  if(starg.ptype==1 && strlen(starg.okfilename)==0)
  { logfile.Write("okfilename is null.\n"); return false; }

  //是否需要检查服务器端文件的时间，true-需要，false-不需要，缺省为false 
