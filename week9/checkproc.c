...
int main(int argc,char *argv[])
{
  ...
  //创建/获取共享内存,键值为SHMKEYP，大小为MAXNUMP哥st_procinfo结构体的大小
  {
  ...
  for(int ii=0;ii<MAXNUMP;ii++)
  {
    ...
    if(iret==-1)
    {
      ...
      memset(shm+ii,0,sizeof(struct st_procinfo));      //共共享内存中删除记录
      continue;
    }
    ...
    //每隔一秒判断一次进程是否存在，一般来说，五秒足够进程退出
    for(int jj=0;jj<5;++jj)
    {
      sleep(1);
      iret=kill(shm[ii].pid,0); //向进程发送信号0，判断是否存在
      if(iret==-1) break;
    }
